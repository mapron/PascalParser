...
#if !defined(PascalLike_COCO_SCANNER_H__)
#define PascalLike_COCO_SCANNER_H__
...
class UTF8Buffer  {
	ByteOrderBuffer& _buf;
	int Read1();
public:
	static const int EoF = COCO_WCHAR_MAX + 1;
	UTF8Buffer(ByteOrderBuffer& buf) : _buf(buf) {}
	int Read();
	int Peek();
	int GetPos();
...
public:
	static const int EoF = COCO_WCHAR_MAX + 1;
	WSBuffer(std::wstring& buf) : _buf(buf),_pos(0) {}
	int Read();
	int Peek();
	int GetPos();
	void SetPos(int value);
...
public:
		WSBuffer *buffer;   // scanner buffer
		std::wstring _buf;
		Scanner();
	~Scanner();
	Token* Scan();
